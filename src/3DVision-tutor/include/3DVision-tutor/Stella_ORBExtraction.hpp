...
struct orb_params
{
    ...
    orb_params(const std::string& name) : orb_params(name, 1.2, 1, 20, 7) {};
...
class ORBExtractor
{
    public:
        ...
        ORBExtractor(){};
        ORBExtractor(const orb_params* orb_params) : orb_params_(orb_params)
        {
            image_pyramid_.resize(orb_params_->num_levels_);
        };

        void ExtractORB_cv(const cv::Mat& image, const bool draw)
        {
            // ORB 특징 검출기 생성
            ...
            {
                cv::Mat img_w_kp;
                cv::drawKeypoints(image, keypoints, img_w_kp);
                cv::imshow("img_w_kp",img_w_kp);
                cv::waitKey(0);
            }
        }
        void ExtractORB_cv(const cv::Mat& image, const int& cell_size, const bool draw)
        {
            // ORB 특징 검출기 생성
            cv::Ptr<cv::Feature2D> orb = cv::ORB::create();
            ...
            cv::Mat rec_image = image.clone();
            int x_gap = std::ceil(static_cast<double>(image.cols) / static_cast<double>(cell_size));
            int y_gap = std::ceil(static_cast<double>(image.rows) / static_cast<double>(cell_size));
            ...
            for(int x = 0; x < cell_size; x++)
            {
                ...
                {
                    int width = x_gap, height = y_gap;
                    if(x*x_gap + x_gap >= image.cols)
                    {
                        width = image.cols-x*x_gap-1;
                        x_up = true;
                    }
                    if(y*y_gap + y_gap >= image.rows)
                    {
                        height = image.rows-y*y_gap-1;
                        y_up = true;
                    }
                        

                    cv::Rect roi(x*x_gap, y*y_gap, width, height);
                    
                    cv::Mat sub_image = image(roi).clone();
                    
                    orb->detectAndCompute(sub_image, cv::noArray(), keypoints, descriptors);
                    if(draw)
                    {
                        for(int i = 0; i < keypoints.size(); i++)
                            cv::circle(rec_image, cv::Point(x*x_gap + keypoints[i].pt.x, y*y_gap + keypoints[i].pt.y), 5, cv::Scalar(0,0,255));

                        cv::rectangle(rec_image, roi, cv::Scalar(0,0,255));
                        cv::imshow("image", rec_image);
        ...
        void compute_fast_keypoints(std::vector<std::vector<cv::KeyPoint>>& all_keypts, const cv::Mat& mask) const
        {
            ...
            //yhj
            // std::cout << "all_keypts.size(): " << all_keypts.size() << std::endl;
            // for(int i = 0; i < all_keypts.size(); i++)
            //     std::cout << "all_keypts[i].size(): " << all_keypts[i].size() << std::endl;
        ...
        // std::cout << "=================start=================" << std::endl;
            for (int64_t level = 0; level < orb_params_->num_levels_; ++level) {
                ...
                // std::cout << "min_border_x: " << min_border_x << std::endl;
                // std::cout << "min_border_y: " << min_border_y << std::endl;
                // std::cout << "max_border_x: " << max_border_x << std::endl;
                // std::cout << "max_border_y: " << max_border_y << std::endl;
                // std::cout << "width: " << width << std::endl;
                // std::cout << "height: " << height << std::endl;
                // std::cout << "num_cols: " << num_cols << std::endl;
                // std::cout << "num_rows: " << num_rows << std::endl;
                // std::cout << image_pyramid_.at(level).size() << std::endl;
                // std::cout << "" << std::endl;
        ...
        #pragma omp critical
                ...
                // std::cout << level << "," << keypts_at_level.size() << "," << keypts_to_distribute.size() << std::endl;
